...
| State | Suggestion |
|-------|-----------|
| No iteration started | Run `/devmeta-ng:plan-iteration 1` to begin |
| Iteration planned, not started | Run `/devmeta-ng:run --all` to execute |
| Iteration in progress, features running | Wait for completion or check blocked items |
| Iteration complete, not reflected | Run `/devmeta-ng:reflect <N>` to capture learnings |
| I&A cycle complete, next not planned | Run `/devmeta-ng:plan-iteration <N+1>` |
