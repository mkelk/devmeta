...
function buildPrompt(feature, ctx) {
  return [
    ...
    `c. Run the SURGICAL test command for the touched code (never the full suite). Fix and re-run until green. Do NOT run a full project build/typecheck (yarn build / full tsc) per feature — the full typecheck runs ONCE at integration (devmeta Phase 4), not N times here.`,
    ...
    `- Do NOT merge. Do NOT touch other feature branches. Merge/integration is devmeta's job after this workflow returns. BEFORE you finish, commit your tk closures on THIS feature branch (git add .tick/ then git commit) so they survive worktree pruning — tk writes per-issue files, so parallel features do not collide on merge; closures left only in the worktree are lost when it is pruned.`,
    ``,
