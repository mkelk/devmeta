---
description: ADAPT Delivery Engine (global) — drives increments to completion
---
...
> **tk mapping:** tk -t epic = ADAPT Feature, tk -t task = ADAPT Task
...
**Current increment:** Defined in `.adapt/current-increment.md`. Follow the pointer to the active increment's `_overview.md` for scope, iterations, and gap analysis.
...
2. `.adapt/current-increment.md` — which increment is active
...
├── Commit metadata to base branch (task, parent: iteration)  ← COMMIT .tick/ AND .adapt/ FILES
...
├── Run /adapt-m-g:reflect N (task)  ← invokes the full 12-step I&A process
└── Plan Iteration N+1 (task)  ← REAL WORK, not a boundary
```

The I&A cycle task invokes `/adapt-m-g:reflect N` as a skill. Do NOT break it into separate tasks -- the skill handles the full sequence internally (code review, docs audit, gap verification, project history update, plan reassessment, and more).

### Re-grounding Task (after every feature)

When you reach a "Re-ground after Feature X" task, do ALL of these before closing it:
1. Update `.adapt/project-history.md` with entry for what you just built
2. Update iteration `status.md` with feature completion
3. Capture any lessons in `.adapt/lessons-learned.md`
4. Run `tk list --parent <iteration-id>` to see where you are in the iteration

### Commit Metadata to Base Branch (after every merge)

After merging a PR and returning to the base branch, `.tick/` and `.adapt/` files will have been modified during orchestration but NOT included in the feature branch PR (since they live on the base branch). You MUST commit them:

```bash
git add .tick/ .adapt/ tsconfig.tsbuildinfo
git status --short   # verify only metadata files staged
git commit -m "Update .tick/ and .adapt/ metadata for iteration N"
```
...
1. Read `.adapt/current-increment.md` to find the active increment directory.
...
Run the environment checks from `.adapt/adapt.md > Environment` if it exists.
If no `.adapt/adapt.md`, skip environment checks.
...
1. Run `/adapt-m-g:plan-iteration N`
2. Planning MUST create the tick structure:
   ...
   - **A "Commit metadata to base branch" task (parent: iteration)** — commits `.tick/` and `.adapt/` files that were modified during orchestration
...
3. Also create the I&A cycle iteration tick:
   ...
   - 2 tasks: "Run /adapt-m-g:reflect N" and "Plan Iteration N+1: read scope, create feature tick structure, begin first task"
...
Create 2 tasks: "Run /adapt-m-g:reflect N" and "Plan Iteration N+1: read scope, create feature tick structure, begin first task". Then `tk next` to start.
...
Read `.adapt/current-increment.md` to find the active increment, then read its `_overview.md` and create the iteration ticks. Each iteration is a top-level epic tick. The `/adapt-m-g:plan-iteration N` command creates the features and tasks within each iteration when it's time to execute.
