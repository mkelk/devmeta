...
```
Current increment's _overview.md (find via .adapt/current-increment.md)
...
- `.adapt/lessons-learned.md` — don't repeat past mistakes
...
If you restructure, update the current increment's `_overview.md` and note the change in `.adapt/project-history.md`. Then continue planning.
...
4. **Note the test strategy** — surgical test commands per task (from `.adapt/adapt.md > Testing` if available)
...
```
.adapt/projects/YYYY-MM-DD-<feature-name>/YYYY-MM-DD-<feature-name>-spec.md
```
...
```
.adapt/projects/YYYY-MM-DD-<feature-name>/context-log.md
```
...
## Spec
\`.adapt/projects/YYYY-MM-DD-<name>/YYYY-MM-DD-<name>-spec.md\`

## Worker Instructions
- Complete tasks in order
- Read \`.adapt/projects/YYYY-MM-DD-<name>/context-log.md\` before starting
- Read \`.adapt/lessons-learned.md\` before starting
...
> 2. Start execution with `/adapt-m-g:run`?
