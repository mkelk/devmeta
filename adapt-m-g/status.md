...
1. Read `.adapt/current-increment.md` → current increment's `_overview.md` — iteration map
...
| State | Suggestion |
|-------|-----------|
| No iteration started | Run `/adapt-m-g:plan-iteration 1` to begin |
| Iteration planned, not started | Run `/adapt-m-g:run --all` to execute |
| Iteration in progress, features running | Wait for completion or check blocked items |
| Iteration complete, not reflected | Run `/adapt-m-g:reflect <N>` to capture learnings |
| I&A cycle complete, next not planned | Run `/adapt-m-g:plan-iteration <N+1>` |
