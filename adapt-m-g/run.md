...
FOR each wave:
  ...
  2. For each feature:
     ...
     b. Read base branch from `<increment-dir>/base-branch` (find increment dir via `.adapt/current-increment.md`)
...
3. Read .adapt/lessons-learned.md — don't repeat known mistakes
4. Read context-log.md for context from previous features
5. If .adapt/adapt.md exists at project root, read it for test commands and additional rules
...
- When you solve a problem, also write it to .adapt/lessons-learned.md
...
### Next Steps
- Address blocked tasks and run `/adapt-m-g:run` again
- Or: run `/adapt-m-g:reflect` if iteration is complete
```
