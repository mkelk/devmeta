...
Before adding E2E to any plan, check for a project-level E2E config:
...
**If `e2e-config.md` exists:** Read it and extract the following fields for use throughout E2E planning:
- `framework` — the E2E framework name (used in descriptions and references)
- `serialization_required` — whether E2E ticks must be chained
- `shared_resource` — what the serialization constraint protects (used in explanatory text)
- `e2e_docs_path` — path to E2E knowledge base docs

**If `e2e-config.md` does NOT exist:** Skip E2E integration for this plan. Warn:
> ⚠️ No `docs/current/e2e/e2e-config.md` found. E2E testing integration is skipped. If this project needs E2E, create a config file first. See `docs/current/e2e/README.md` for guidance.

#### E2E Serialization Constraint

**Read the `serialization_required` field from `e2e-config.md`.**

**If `serialization_required` is true:**

E2E tests CANNOT run in parallel. The config's `shared_resource` field explains why (e.g., "Android emulator — only one app instance at a time"). Only ONE E2E test can run at a time.

**When creating E2E ticks, you MUST chain them with `--blocked-by` dependencies:**
...
**Why this matters:**
- The shared resource can only serve one test at a time
- Parallel E2E tests will fight for it and both fail
- Serial execution ensures reliable, reproducible results

**If `serialization_required` is false:** E2E ticks can run in parallel — no special chaining needed.

**Non-E2E tasks CAN always run in parallel with E2E tasks:**
- Unit tests, code review, documentation can run alongside E2E
- Only E2E-to-E2E must be serialized (when config requires it)

**Reference the E2E knowledge base** (use `e2e_docs_path` from config, defaults to `docs/current/e2e/`):
- Hub: `<e2e_docs_path>/README.md`
- UI epic structure: `<e2e_docs_path>/ui-epic-structure.md`
- Project directory pattern: `<e2e_docs_path>/project-e2e-directory.md`
- Tick quick reference: `<e2e_docs_path>/tick-quick-reference.md`

**Step 1: Auto-create the project e2e/ directory**

When the E2E config exists and the plan includes E2E ticks, **auto-create the project e2e/ directory** using the templates from `docs/current/e2e/project-e2e-directory.md`:
...
├── COOKBOOK.md       # Project-specific patterns (from cookbook-template.md)
├── LOG.md           # Test run history
└── flows/           # Project-specific test files
```

Create these files using the templates defined in `docs/current/e2e/project-e2e-directory.md` (see "Template Files" section). For COOKBOOK.md, use `docs/current/e2e/cookbook-template.md` as the base and fill in project-specific values.

**This replaces the old pattern of making the first tick create these files** — they are now created at plan time so all ticks have them from the start.
...
**E2E Framework:** <framework from config>
...
**E2E Knowledge Base:** \`<e2e_docs_path>/\`"
```
...
**Quick reference:** <e2e_docs_path>/tick-quick-reference.md
